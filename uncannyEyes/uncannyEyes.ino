...
typedef struct
{                   // Struct is defined before including config.h --
...
void activateEye()
{
  ...
  for (e = 0; e < NUM_EYES; e++)
  { // Another pass, after all screen inits
  ...
  for (e = 0; e < NUM_EYES; e++)
  {
    ...
    eye[e].display->writeData(e ? rotateOLED[eyeInfo[e].rotation & 3] : mirrorOLED[eyeInfo[e].rotation & 3]);
  }
#endif
...
#ifdef ARDUINO_ARCH_SAMD
    while (dma_busy)
      ; // Wait for prior DMA xfer to finish
...
#ifdef ARDUINO_ARCH_SAMD
  while (dma_busy)
    ; // Wait for last scanline to transmit
#else
